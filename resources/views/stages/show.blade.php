<x-app-layout>
     ...
     <div class="py-8">
          <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
               {{-- Breadcrumb --}}
               <div class="mb-6">
                    <a href="{{ route('stages.index') }}" class="text-slate-400 hover:text-white text-sm transition">←
                         Back to Stages</a>
               </div>
               ...
               <div class="bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10 mb-6">
                    ...
                    <div class="mt-6">
                         <form action="{{ route('quiz.start', $stage) }}" method="POST">
                              @csrf
                              <button type="submit"
                                   class="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white py-3 px-6 rounded-xl font-bold text-lg transition-all duration-200 shadow-lg hover:shadow-purple-500/30">
               ...
               @if($bestAttempt)
                    <div class="bg-emerald-500/10 rounded-2xl p-6 border border-emerald-500/20 mb-6">
                         <h3 class="text-lg font-bold text-emerald-400 mb-2">🏆 Best Score</h3>
               ...
               @if($attemptHistory->count() > 0)
                    <div class="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                         <h3 class="text-lg font-bold text-white mb-4">📊 Attempt History</h3>
                         <div class="space-y-2">
                              @foreach($attemptHistory as $attempt)
                                   <div class="flex items-center justify-between p-3 rounded-xl bg-white/5">
                                        ...
                                        <div class="flex items-center space-x-3">
                                             ...
                                             <span
                                                  class="text-xs px-2 py-1 rounded-full {{ $attempt->passed ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400' }}">
