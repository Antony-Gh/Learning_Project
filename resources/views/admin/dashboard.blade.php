<x-app-layout>
     ...
     <div class="py-8">
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
               <h1 class="text-3xl font-bold text-white mb-2">⚙️ Admin Dashboard</h1>
               ...
               {{-- Recent Attempts --}}
               <div class="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                    <h2 class="text-xl font-bold text-white mb-4">📝 Recent Attempts</h2>
                                   ...
                                   @foreach($recentAttempts as $attempt)
                                        <tr class="border-b border-white/5 hover:bg-white/5">
                                             ...
                                             <td class="px-4 py-3 text-white">
                                                  {{ $attempt->score }}/{{ $attempt->total_questions }}</td>
                                             <td class="px-4 py-3">
                                                  <span
                                                       class="px-2 py-1 rounded-full text-xs {{ $attempt->passed ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400' }}">
